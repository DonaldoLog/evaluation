    ...
    <div>
        <b-card
            ...
            <b-card-body class="row">
                <div class="form-form col-12">
                    <label for="name">Nombre</label>
                    <input type="text" class="form-control form-control-sm" id="name" name="name" v-model="form.name" v-validate="{ required: true }">
                ...
                <br>
                <br>
                <button v-show="!edit" class="btn btn-secondary col-2 offset-5" @click="storeForm">Guardar</button>
                <button v-show="edit" class="btn btn-secondary col-2 offset-5" @click="updateForm">Actualizar</button>
            </b-card-body>
            </div>
                                    ...
                                    <table class="table table-striped responsive">
                                        ...
                                        <tbody>
                                            <template v-if="tableData.length > 0">
                                                <template v-for="(form, index) in tableData">
                                                        <tr :key="index">
                                                            <td> {{ form.name }} </td>
                                                            <td>
                                                                <button class="btn btn-secondary" @click="editForm(form.id)" title="Editar"> <i class="fa fa-edit"></i></button>
                                                                <button class="btn btn-secondary" @click="destroyForm(form)" title="Eliminar"><i class="fa fa-trash"></i></button>
                                                                <a class="btn btn-secondary" :href="mainUrl+'/forms/admin/'+form.id" target="_blank" title="Admin"><i class="fas fa-tools"></i></a>
                                                            </td>
                                                        </tr>
                                                </template>
                                            </template>
...
Vue.component('v-select', vSelect)
  ...
  export default {
    ...
    methods: {
        ...
        fetchData() {
            let dataFetchUrl = `${this.mainUrl}/forms/data`;
        ...
        storeForm () {
            this.$validator.validate().then(valid => {
                if (valid) {
                    this.loading = true
                    axios.post(`${this.mainUrl}/forms/store`, {
                        form: this.form,
                        career: this.career
                    })
                    .then((response) => {
        ...
        editForm (formId) {
            this.loading = true
            this.edit = true
            axios.get(`${this.mainUrl}/forms/${formId}`)
            .then(res => {
                this.loading = false
                if (res.data.success) {
                    this.form = {
        ...
        updateForm () {
            this.loading = true
            axios.post(`${this.mainUrl}/forms/update`, { form: this.form, career: this.career })
            .then(res => {
                this.loading = false
                if (res.data.success) {
                    Vue.swal({
                        title: 'Éxito',
                        text: "Cuestionario actualizada correctamente.",
                        type: 'success',
                        showCancelButton: false,
                        confirmButtonColor: '#3085d6',
                        confirmButtonText: 'Aceptar',
        ...
        destroyForm (form) {
             Vue.swal({
                title: '¿Estas seguro de eliminar el cuestionario '+form.name+'?',
                ...
                allowEscapeKey: false
                    ...
                    if (result.value) {
                        Vue.swal({
                            title: 'Eliminara el cuestionario ' + form.name + '.',
                            type: 'warning',
                            showCancelButton: true,
                            confirmButtonColor: '#3085d6',
                            cancelButtonColor: '#d33',
                            ...
                            allowEscapeKey: false
                                ...
                                if (result.value) {
                                    this.loading = true
                                    axios.post(`${this.mainUrl}/forms/destroy`, { form: form })
                                    .then(res => {
                                        this.loading = false
                                        if (res.data.success) {
                                            Vue.swal({
